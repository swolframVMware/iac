...
DOCUMENTATION = r"""
---
module: sddc_manager_sddc_info
short_description: Retrieves information for the SDDC from SDDC Manager.
description:
    - This module retrieves information for the SDDC from SDDC Manager for VMware Cloud Foundation.
author:
...
EXAMPLES = r"""
- name: Get SDDC information from SDDC Manager
  broadcom.vcf.sddc_manager_sddc_info:
...
RETURN = r"""
msg:
    description: Error message if no SDDC is found
    returned: on failure
    type: str
    sample: "No SDDC found in SDDC Manager sddc-manager.example.com."
meta:
    description: SDDC information response from SDDC Manager API
    returned: on success
    type: dict
    sample:
        elements:
...
class SddcManagerSDDCInfo:
    """This class represents the SDDC information process in SDDC Manager.
    ...
    Methods:
        get_sddc_info(self): Retrieves information for the SDDC from SDDC Manager.
        run(self): Runs the SDDC information retrieval process.
    ...
    def get_sddc_info(self):
        """Retrieves information for the SDDC from SDDC Manager."""
        try:
            api_response = self.api_client.get_sddc_manager_info()
            response = api_response

            if not response["elements"]:
                self.module.fail_json(
                    msg=f"No SDDC found in SDDC Manager {self.sddc_manager_hostname}."
                )
            else:
    ...
    def run(self):
        """Runs the SDDC information retrieval process."""
        self.get_sddc_info()
...
def main():
    ...
    sddc_info = SddcManagerSDDCInfo(module)
    sddc_info.run()
