...
short_description: Retrieves the NTP configuration from SDDC Manager.
description:
    - This module retrieves the Network Time Protocol (NTP) configuration from SDDC Manager for VMware Cloud Foundation.
author:
...
RETURN = r"""
ntp:
    description: NTP status information
    returned: on success
    type: dict
    sample:
        {
...
class SddcManagerNtpInfo:
    """This class represents the NTP configuration information retrieval in SDDC Manager.
    ...
    Methods:
        get_ntp_configuration(self): Retrieves NTP configuration from SDDC Manager.
        run(self): Runs the NTP configuration retrieval process.
    ...
    def get_ntp_configuration(self):
        """Retrieves NTP configuration from SDDC Manager."""
        try:
            api_response = self.api_client.get_ntp_configuration()
            self.module.exit_json(changed=False, ntp=api_response)

        except VcfApiException as e:
            self.module.fail_json(msg=f"Error retrieving NTP configuration: {e}")

    def run(self):
        """Runs the NTP configuration retrieval process."""
        self.get_ntp_configuration()
...
def main():
    ...
    ntp_info = SddcManagerNtpInfo(module)
    ntp_info.run()
