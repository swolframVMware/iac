...
short_description: Retrieves the DNS configuration from SDDC Manager.
description:
    - This module retrieves the Domain Name System (DNS) configuration from SDDC Manager for VMware Cloud Foundation.
author:
...
RETURN = r"""
dns:
    description: DNS status information
    returned: on success
    type: dict
    sample:
        {
...
class SddcManagerDnsInfo:
    """This class represents the DNS configuration information retrieval in SDDC Manager.
    ...
    Methods:
        get_dns_configuration(self): Retrieves DNS configuration from SDDC Manager.
        run(self): Runs the DNS configuration retrieval process.
    ...
    def get_dns_configuration(self):
        """Retrieves DNS configuration from SDDC Manager."""
        try:
            api_response = self.api_client.get_dns_configuration()
            self.module.exit_json(changed=False, dns=api_response)

        except VcfApiException as e:
            self.module.fail_json(msg=f"Error retrieving DNS configuration: {e}")

    def run(self):
        """Runs the DNS configuration retrieval process."""
...
def main():
    ...
    dns_info = SddcManagerDnsInfo(module)
    dns_info.run()
